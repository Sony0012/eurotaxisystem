...
Route::get('/web-notifications/native-poll', function(\Illuminate\Http\Request $request) {
    try {
        ...
        $alerts = \Illuminate\Support\Facades\Cache::remember("native_poll_{$userId}", 10, function() use ($userId) {
            return \Illuminate\Support\Facades\DB::table('system_alerts')
...
Route::middleware(['auth', 'page_access'])->group(function () {
    ...
    Route::get('/analytics/expenses-breakdown', [AnalyticsController::class, 'expensesBreakdown'])->name('analytics.expenses-breakdown');
    ...
    Route::get('/staff/drivers', [StaffController::class, 'driversIndex'])->name('staff.drivers');
    ...
    Route::get('/support-center', [\App\Http\Controllers\SupportManagementController::class, 'index'])->name('support.index');
    ...
    Route::post('/announcements/{id}/toggle-pin', [\App\Http\Controllers\AnnouncementController::class, 'togglePin'])->name('announcements.toggle-pin');
});
...
Route::get('/offline-fallback', function() {
    ...
    <head>
        ...
        <title>Naka-Offline - EuroTaxi</title>
    ...
    <body class="flex items-center justify-center min-h-screen p-6 text-slate-100">
        <div class="w-full max-w-md p-8 text-center bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl">
            <div class="flex justify-center mb-6">
                <div class="p-5 bg-blue-500/10 text-blue-400 rounded-full animate-pulse border border-blue-500/20">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 1l22 22"/><path d="M16.72 11.06A10.94 10.94 0 0 1 19 12.5"/><path d="M5 12.5a10.94 10.94 0 0 1 5.17-2.39"/><path d="M10.71 5.05A16 16 0 0 1 22.58 9"/><path d="M1.42 9a15.91 15.91 0 0 1 4.7-2.88"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><path d="M12 20h.01"/></svg>
                </div>
            </div>
            <h1 class="text-2xl font-bold tracking-tight text-white mb-2">Naka-offline Ka</h1>
            <p class="text-sm text-slate-400 leading-relaxed mb-8">
                Mukhang nawalan ng signal ang iyong device. Naka-save muna offline ang lahat ng iyong ginagawa at kusang isasabay kapag bumalik ang iyong koneksyon.
            </p>
            <button onclick="window.location.reload()" class="w-full py-3.5 px-6 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-2xl transition duration-200 shadow-lg shadow-blue-500/25 active:scale-95">
