        ...
        try {
            ...
            box.innerHTML = messages.map((m, index) => {
                ...
                const tooltipText = tooltipParts.join("\n");
                    ...
                    reactionsHtml = `
                        <div class="absolute -bottom-2 ${m.is_mine ? '-left-2' : '-right-2'} bg-white text-gray-800 rounded-full px-1 py-0.5 text-[11px] leading-none flex items-center justify-center shadow-md cursor-pointer z-20 border border-gray-100" onclick="chatShowReactionPicker(event, ${m.id})">
                ...
                return `
                <div class="flex ${m.is_mine ? 'justify-end' : 'justify-start'} ${marginClass} group pl-1 pr-1 pb-4">
...
window.chatPreviewMedia = function(type, url) {
    ...
    if (type === 'image') {
        container.innerHTML = `<img src="${url}" class="max-w-full max-h-[90vh] rounded-xl shadow-2xl" onclick="event.stopPropagation()">`;
    } else if (type === 'video') {
        container.innerHTML = `<video src="${url}" controls autoplay class="max-w-full max-h-[90vh] rounded-xl shadow-2xl" onclick="event.stopPropagation()"></video>`;
    }
...
window.chatShowReactionPicker = function(event, messageId) {
    ...
    if (drawer) {
        ...
        if (x > rect.right - 220) x = rect.right - 220; // picker is roughly 200px wide
...
<div id="chatMediaLightbox" 
    ...
    <div id="chatMediaContainer" class="max-w-[90vw] max-h-[90vh] flex items-center justify-center">
