...
function tryConnect(attempt = 1) {
    ...
    conn.on('ready', () => {
        conn.sftp((err, sftp) => {
            sftp.fastPut('probe_aksh5.php', BASE + '/probe_aksh5.php', {}, () => {
                console.log('Uploaded probe_aksh5.php, running...\n');
                conn.exec(`cd ${BASE} && php probe_aksh5.php 2>&1`, (e2, stream) => {
                    stream
                        .on('close', () => conn.exec(`rm -f ${BASE}/probe_aksh5.php`, () => conn.end()))
