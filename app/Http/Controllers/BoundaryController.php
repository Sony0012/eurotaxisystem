...
class BoundaryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $units = DB::table('units')
            ...
            ->select('id', 'uuid', 'plate_number', 'make', 'model', 'year', 'boundary_rate', 'coding_day', 'driver_id', 'secondary_driver_id', 'current_turn_driver_id', 'last_swapping_at', 'shift_deadline_at')
            ->orderBy('plate_number')
            ->get()
        ...
        $pendingDebts = DB::table('driver_behavior')
            ...
            ->whereNull('deleted_at')
            ->select('id', 'uuid', 'driver_id', 'incident_type', 'incident_date', 'description', 'remaining_balance')
    ...
    public function store(Request $request)
    {
        ...
        if ($action === 'update_boundary') {
            ...
            if ($id > 0 && $is_valid_amount) {
                $boundary = Boundary::where('uuid', $id)->first();
                if (!$boundary) {
    ...
    {
        $boundary = Boundary::where('uuid', $id)->firstOrFail();
        $plate = DB::table('units')->where('id', $boundary->unit_id)->value('plate_number');
        $date = $boundary->date;
        $boundary->delete();
