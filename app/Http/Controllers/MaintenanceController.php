...
class MaintenanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $drivers = DB::table('drivers')
            ...
            ->select('id', 'uuid', DB::raw('CONCAT(first_name, " ", last_name) as name'), 'nickname')
            ->orderBy('first_name')
            ->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        return DB::transaction(function () use ($id, $data, $parsed, $request) {
            $maintenance = Maintenance::where('uuid', $id)->firstOrFail();
    ...
    {
        $maint = Maintenance::where('uuid', $id)->firstOrFail();
