    ...
    public function boot()
    {
        ...
        \Illuminate\Support\Facades\View::composer('layouts.app', function ($view) {
            ...
            try {
                ...
                // ─── 1. AUTO-GENERATE SYSTEM ALERTS (RUNS ON ANY PAGE LOAD) ───────────
                
                // A. Coding Notice Alert
                ...
                if ($isWeekday) {
                    ...
                    if (!$codingAlertExists) {
                        ...
                        $codingUnitsCount = $allUnits->filter(function($u) use ($todayDay) {
                            // Extract last digit for coding day check
                ...
                // B. Overdue/Missing Unit Alert (> 24 hours)
                        ...
                        if ($suspectId) {
                            ...
                            if (!$existingViolation) {
                                \Illuminate\Support\Facades\DB::table('driver_behavior')->insert([
                                    ...
                                    'severity' => 'high', 'description' => "Auto-logged [Flagdown]: Unit {$mu->plate_number} is overdue for >48 hours.",
                ...
                // ─── 2. GATHER ALL NOTIFICATIONS ────────────────────────────────────

                // A. Flagged "At Risk" Units
                $flagged = \Illuminate\Support\Facades\DB::table('units')
                ...
                // B. System Alerts (Violations, Coding, Missing)
                ...
                $todayMaint = \Illuminate\Support\Facades\DB::table('maintenance')
                    ...
                    ->where('maintenance.date_started', $today)
                ...
                foreach($todayMaint as $tm) {
                    $headerNotifications[] = [
                        'type' => 'maintenance_today', 'title' => 'Maintenance Today', 'message' => "Unit {$tm->plate_number} is scheduled for " . ucfirst($tm->maintenance_type),
                ...
                foreach ($lowStock as $p) {
                    ...
                    $headerNotifications[] = [
                        ...
                        'message' => "Only {$qty} items remaining in inventory.", 'url' => route('maintenance.index', ['open_inventory' => 1]),
                ...
                // Fail silently to avoid crashing the whole app if DB is messy
