...
<div class="bg-white rounded-lg shadow p-4 mb-5">
    <form onsubmit="event.preventDefault(); performMaintenanceSearch(1);" class="flex flex-wrap gap-3">
        ...
        <input type="date" name="date_filter" value="{{ $date_filter ?? '' }}" onchange="performMaintenanceSearch(1)" 
            class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none text-gray-600">
        <select name="status" onchange="performMaintenanceSearch(1)" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
        ...
        <select name="type" onchange="performMaintenanceSearch(1)" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none">
...
<div id="tableContainer" class="transition-opacity duration-200">
    @include('maintenance.partials._maintenance_table')
</div>
...
function validateMaintForm(mode) {
    ...
    // No strict parts or costs requirement for general check-ups.
...
function filterDriverSuggestions(inputId, dropdownId, suggestedIds) {
    ...
    // If the user hasn't typed anything new and it's just the auto-filled name, show all options
    if (inputEl.dataset.autofilled && searchTerm === inputEl.dataset.autofilled) {
        searchTerm = '';
    }
    ...
    options.forEach(opt => {
        ...
        if (isSuggested) {
            opt.classList.add('recommended-option', 'bg-green-50', 'border-l-4', 'border-green-500', 'hover:bg-green-100');
            ...
            opt.classList.remove('recommended-option', 'bg-green-50', 'border-l-4', 'border-green-500', 'hover:bg-green-100');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initSearchableDropdown('addUnitDisplay', 'addUnitDropdown', 'unit-option', (data) => {
        ...
        // Auto-fill the primary driver if available
        if (driverId && driverId !== 'null' && driverName) {
            driverDisplay.value = driverName;
            driverDisplay.dataset.autofilled = driverName.toLowerCase();
            driverHidden.value = driverId;
        } else {
            driverDisplay.value = '';
            driverDisplay.dataset.autofilled = '';
            driverHidden.value = '';
        }
...
// Live filtering for Maintenance Table with AJAX Fetch
let searchDebounceTimer;
function filterMaintenanceTable(query) {
    const q = query.toLowerCase();
    const rows = document.querySelectorAll('tr.modern-row');
    rows.forEach(row => {
    ...
    searchDebounceTimer = setTimeout(() => {
        performMaintenanceSearch(1);
    }, 500); // reduced to 500ms since AJAX is non-blocking
}

function performMaintenanceSearch(page = 1) {
    ...
    const spinner = document.getElementById('maintLoadingSpinner');
    ...
    const finalUrl = `{{ route('maintenance.index') }}?search=${encodeURIComponent(query)}&date_filter=${dateFilter}&status=${status}&type=${type}&page=${page}`;
