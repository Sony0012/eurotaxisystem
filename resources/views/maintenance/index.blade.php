                ...
                onclick="openViewMaint({{ $r->uuid }})">
                    ...
                    <td class="px-4 py-3">
                        @if(!$r->mechanic_name || $r->mechanic_name === '—')
                            <button onclick="openEditMaint(document.querySelector('button[data-id=\'{{ $r->uuid }}\']')); event.stopPropagation();" data-id="{{ $r->uuid }}" class="flex items-center gap-1.5 px-2 py-1 border border-dashed border-gray-300 rounded-lg text-[10px] font-bold text-gray-500 hover:text-blue-600 hover:border-blue-400 hover:bg-blue-50 transition w-max">
                    ...
                    <td class="px-4 py-3" onclick="event.stopPropagation()">
                        <div class="flex gap-2">
                            {{-- Toggle Complete --}}
                            <form method="POST" action="{{ route('maintenance.toggle-complete', $r->uuid) }}">
                                @csrf
                            ...
                            <form method="POST" action="{{ route('maintenance.toggle-in-progress', $r->uuid) }}">
                                @csrf
                                <button type="submit"
                            ...
                            <button onclick="openEditMaint(this)" data-id="{{ $r->uuid }}" class="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50 transition"><i data-lucide="edit" class="w-4 h-4"></i></button>
                            <form method="POST" action="{{ route('maintenance.destroy', $r->uuid) }}" onsubmit="return confirm('Delete?')">
                                @csrf @method('DELETE')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <div id="addDriverDropdown" class="search-dropdown hidden">
                            ...
                            <div class="search-option driver-option" data-id="{{ $d->uuid }}" data-name="{{ $d->name }}">
                    ...
                    <div class="relative mb-3">
                        ...
                        <div id="addPartDropdown" class="search-dropdown hidden">
                            ...
                            <div class="search-option part-option {{ $isOut ? 'opacity-50 grayscale cursor-not-allowed' : '' }}" 
                                data-id="{{ $p->uuid }}" data-name="{{ $p->name }}" data-price="{{ $p->price }}" data-qty="{{ $p->stock_quantity ?? 0 }}">
                                <div class="flex justify-between items-center">
                                    <div class="flex-1">
                            ...
                            <tr class="hover:bg-gray-50/60 transition parts-row" data-name="{{ strtolower($p->name) }}" data-supplier="{{ strtolower($p->supplier ?? '') }}">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex justify-end gap-1">
                                        <button onclick="editCatalogPart({{ $p->uuid }}, '{{ addslashes($p->name) }}', {{ $p->price }}, {{ $p->stock_quantity ?? 0 }}, '{{ addslashes($p->supplier ?? '') }}')"
                                            title="Add Stock"
                                            class="p-2 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
                                            <i data-lucide="package-plus" class="w-4 h-4"></i>
                                        </button>
                                        <button onclick="deletePart({{ $p->uuid }}, this)"
                                            title="Delete"
                        ...
                        <tr class="hover:bg-gray-50 transition group">
                            ...
                            <td class="py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    <button onclick="editSupplier({{ $s->uuid }}, '{{ addslashes($s->name) }}', '{{ addslashes($s->contact_person) }}', '{{ $s->phone_number }}')" 
                                    ...
                                    <button onclick="deleteSupplier({{ $s->uuid }}, this)" 
