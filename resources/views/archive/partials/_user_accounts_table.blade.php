        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-bold text-gray-900">{{ $u->full_name }}</div>
                    @if($u->role === 'driver' && $u->driver)
                        <div class="text-[10px] text-indigo-600 font-semibold italic">Linked to Driver: {{ $u->driver->full_name }}</div>
                    @endif
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                    <span class="inline-flex text-[10px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-tight">{{ $u->role }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button type="button"
                        onclick="archiveRestore('{{ route('archive.restore', ['type' => 'user', 'id' => $u->uuid]) }}')"
                    ...
                    <button type="button"
                        onclick="archiveForceDelete('{{ route('archive.forceDelete', ['type' => 'user', 'id' => $u->uuid]) }}')"
            ...
            <tr>
                <td colspan="5" class="px-6 py-16 text-center">
                    <div class="flex flex-col items-center gap-3 text-gray-400">
                        ...
                        <p class="text-sm font-medium">No archived user accounts found.</p>
                    </div>
                </td>
            </tr>
            @endforelse
