<div class="overflow-x-auto w-full">
    <table class="w-full divide-y divide-slate-100">
        ...
        <tbody class="bg-white divide-y divide-slate-100">
            ...
            <tr class="hover:bg-slate-50/80 transition-colors" data-id="{{ $case->id }}">
                <td class="w-10 px-4 sm:px-6 py-5 text-center">
                    <input type="checkbox" class="archive-row-cb rounded border-slate-300 text-amber-500 focus:ring-amber-400 h-4 w-4 cursor-pointer" value="{{ $case->id }}" data-id="{{ $case->id }}">
                </td>
                ...
                <td class="px-6 sm:px-8 py-5 whitespace-nowrap text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button type="button"
                            onclick="archiveRestore('{{ route('archive.restore', ['type' => 'franchise_case', 'id' => $case->id]) }}', this)"
                        ...
                        <button type="button"
                            onclick="archiveForceDelete('{{ route('archive.forceDelete', ['type' => 'franchise_case', 'id' => $case->id]) }}', this)"
