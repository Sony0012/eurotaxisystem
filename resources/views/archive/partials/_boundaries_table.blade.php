    ...
    <table class="w-full divide-y divide-slate-100">
        <thead class="bg-slate-50/70">
            <tr>
                ...
                <th class="px-6 sm:px-8 py-4 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Boundary Date</th>
        ...
        <tbody class="bg-white divide-y divide-slate-100">
            @forelse($items as $b)
            <tr class="hover:bg-slate-50/80 transition-colors" data-id="{{ $b->id }}">
                <td class="w-10 px-4 sm:px-6 py-5 text-center">
                    <input type="checkbox" class="archive-row-cb rounded border-slate-300 text-amber-500 focus:ring-amber-400 h-4 w-4 cursor-pointer" value="{{ $b->id }}" data-id="{{ $b->id }}">
                </td>
                <td class="px-6 sm:px-8 py-5 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-black text-xs shadow-2xs shrink-0">
                            <i data-lucide="wallet" class="w-4 h-4"></i>
                        </div>
                        <span class="text-sm font-black text-slate-900">Unit #{{ $b->unit_id }}</span>
                    </div>
                </td>
                <td class="px-6 sm:px-8 py-5 whitespace-nowrap text-sm font-black text-emerald-600">
                    {{ formatCurrency($b->boundary_amount) }}
                </td>
                ...
                <td class="px-6 sm:px-8 py-5 whitespace-nowrap">
                    <span class="inline-flex items-center gap-1.5 text-xs font-bold text-rose-700 bg-rose-50 border border-rose-200 px-2.5 py-1 rounded-xl">
                        ...
                        {{ $b->deleted_at->format('M d, Y h:i A') }}
                    </span>
                </td>
                ...
                <td colspan="6" class="px-6 sm:px-8 py-16 text-center">
                    <div class="flex flex-col items-center gap-3 text-slate-400">
                        <div class="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center">
                            <i data-lucide="wallet" class="w-8 h-8 opacity-40"></i>
                        </div>
                        <p class="text-sm font-bold text-slate-700">No archived boundary records found</p>
                        <p class="text-xs text-slate-400">Archived boundaries will appear here.</p>
                    </div>
                </td>
            </tr>
            @endforelse
