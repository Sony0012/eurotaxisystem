        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <form action="{{ route('archive.restore', ['type' => 'boundary', 'id' => $b->uuid]) }}" method="POST" class="inline-block">
                        @csrf
                    ...
                    <button type="button"
                        onclick="archiveForceDelete('{{ route('archive.forceDelete', ['type' => 'boundary', 'id' => $b->uuid]) }}')"
            ...
            <tr>
                <td colspan="5" class="px-6 py-16 text-center">
                    <div class="flex flex-col items-center gap-3 text-gray-400">
                        <i data-lucide="wallet" class="w-12 h-12 opacity-30"></i>
                        <p class="text-sm font-medium">No archived boundary records found.</p>
                    </div>
                </td>
            </tr>
            @endforelse
