...
@section('page-subheading', 'Financial performance and operational insights')
...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow card-hover p-5">
            ...
            <p class="text-2xl font-bold text-green-600">{{ formatCurrency($total_boundary) }}</p>
        </div>
        <div class="bg-white rounded-lg shadow card-hover p-5">
            <p class="text-sm text-gray-500">Total Expenses</p>
            <p class="text-2xl font-bold text-red-600">{{ formatCurrency($total_expenses) }}</p>
        </div>
        <div class="bg-white rounded-lg shadow card-hover p-5">
            <p class="text-sm text-gray-500">Net Income</p>
            <p class="text-2xl font-bold {{ $net_income >= 0 ? 'text-green-600' : 'text-red-600' }}">
                {{ formatCurrency($net_income) }}</p>
        </div>
        <div class="bg-white rounded-lg shadow card-hover p-5">
            <p class="text-sm text-gray-500">Active Drivers</p>
            <p class="text-2xl font-bold text-blue-600">{{ $active_drivers }}</p>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($top_units as $i => $u)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $u->unit_number }}</div>
                                <div class="text-xs text-gray-500">{{ $u->plate_number }}</div>
                            </td>
