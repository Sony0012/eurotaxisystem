        ...
        <div class="lg:col-span-8">
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <form action="{{ route('announcements.store') }}" method="POST" id="broadcastComposerForm" class="p-6 sm:p-8 space-y-6">
                    ...
                    <!-- ── Integrated Meeting Scheduler / Calendar UI ── -->
                    <div class="rounded-2xl border border-slate-200/90 bg-gradient-to-br from-slate-50/80 to-slate-100/40 p-5 sm:p-6 space-y-5">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] text-slate-500">Pick the active start date and display expiration date.</p>
                                </div>
                            </div>
                            <!-- Duration badge -->
                        ...
                        <!-- Calendar Grid & Date Selection Columns -->
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs">
                            ...
                            <div class="flex flex-col">
                                <div class="flex items-center justify-between mb-3">
                                    <button type="button" onclick="prevCalendarMonth()" class="p-2 rounded-xl hover:bg-slate-100 text-slate-600 transition-colors" aria-label="Previous Month">
                                        <i data-lucide="chevron-left" class="w-4 h-4"></i>
                                    </button>
                                    <h4 id="calendarMonthYear" class="text-sm font-bold text-slate-800 tracking-tight">July 2026</h4>
                            ...
                            <div class="flex flex-col justify-between space-y-4">
                                <div class="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50/70">
                                            <span id="displayStartDateText" class="text-xs font-bold text-slate-800">Aug 22, 2026</span>
                                            <span class="text-[10px] font-bold text-amber-700 bg-amber-100/80 px-2.5 py-0.5 rounded-md">12:00 AM</span>
                                        </div>
                                    </div>

                                    <!-- End Date Display -->
                                    <div>
                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">End date / Display Until*</label>
                                        <div class="flex items-center justify-between p-3 rounded-xl border border-slate-200 bg-slate-50/70">
                                            <span id="displayEndDateText" class="text-xs font-bold text-slate-800">Aug 29, 2026</span>
                                            <span class="text-[10px] font-bold text-amber-700 bg-amber-100/80 px-2.5 py-0.5 rounded-md">11:59 PM</span>
                                        </div>
                                    </div>
                                ...
                                <div class="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                                    <span id="schedulerEventSummary" class="text-slate-500 font-semibold truncate">Event: Aug 22 - Aug 29, 2026</span>
        ...
        <div class="lg:col-span-4 space-y-6">
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 space-y-5">
                ...
                <div class="rounded-2xl border border-amber-200/80 bg-gradient-to-br from-amber-50/90 via-yellow-50/50 to-amber-100/40 p-4 space-y-3 shadow-2xs relative overflow-hidden">
                    ...
                    <div class="pt-3 border-t border-amber-200/60 flex items-center justify-between text-[10px] font-bold text-slate-500">
                        <span class="flex items-center gap-1">
                            <i data-lucide="clock-3" class="w-3 h-3 text-amber-600"></i>
                            <span id="previewSchedule">Scheduled: Aug 22 - Aug 29</span>
                        </span>
                        <span class="px-2 py-0.5 bg-amber-200/80 text-amber-900 rounded-md text-[9px] font-black uppercase">
                            Active
                        </span>
                    </div>
                </div>
                ...
                <div class="p-3.5 bg-slate-50 rounded-2xl border border-slate-100 text-xs text-slate-500 leading-relaxed flex items-start gap-2.5">
                    ...
                    <span>Drivers receive instant push alerts and see this banner pinned at the top of their dashboard until the expiration date.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all duration-300 scale-95 opacity-0" id="viewModalContainer">
        ...
        <div class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Display Until</label>
        ...
        <form id="editForm" method="POST">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Start Date</label>
                        <input type="date" name="start_date" id="editStartDate" required
                            class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all outline-none text-xs font-bold text-slate-800">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Display Until</label>
...
<script>
    // ── INTERACTIVE MEETING SCHEDULER IMPLEMENTATION ───────────────────────
    ...
    selEndDate.setHours(23,59,59,999);
    ...
    function renderCalendar() {
        ...
        for (let i = firstDayIndex - 1; i >= 0; i--) {
            ...
            btn.className = 'h-9 w-full flex items-center justify-center text-xs text-slate-300 select-none';
        ...
        for (let day = 1; day <= totalDays; day++) {
            ...
            btn.type = 'button';
            btn.className = 'relative h-9 w-full rounded-xl flex items-center justify-center text-xs font-bold transition-all';
            ...
            if (isStart || isEnd) {
                ...
                btn.className += ' text-amber-600 font-black border border-amber-300';
            } else {
    ...
    function updateSchedulerDisplay() {
        ...
        if (selStartDate && selEndDate) {
            ...
            summaryEl.textContent = `Event: ${formatShortDate(selStartDate)} - ${formatShortDate(selEndDate)} (${badgeText})`;
    ...
    function setSchedulerPreset(days) {
        if (!selStartDate) selStartDate = new Date();
