...
@section('title', 'Decision Management - Euro System')
@section('page-heading', 'Decision Management')
@section('page-subheading', 'Franchise Cases & Applications')
...
<div class="mb-4 p-4 bg-yellow-50 border border-yellow-300 rounded-lg flex items-center gap-3">
    ...
    <span class="text-sm text-yellow-800 font-medium">{{ $stats['expiring_soon'] }} case(s) expiring within 30 days.</span>
</div>
@endif
@if($stats['expired'] > 0)
<div class="mb-4 p-4 bg-red-50 border border-red-300 rounded-lg flex items-center gap-3">
    <i data-lucide="x-circle" class="w-5 h-5 text-red-600"></i>
    <span class="text-sm text-red-800 font-medium">{{ $stats['expired'] }} case(s) have already expired.</span>
</div>
@endif

{{-- Toolbar --}}
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <form method="GET" action="{{ route('decision-management.index') }}" class="flex flex-col md:flex-row gap-3">
        <input type="text" name="search" value="{{ $search }}" placeholder="Search applicant, case no, type..."
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-5 py-4 font-semibold text-gray-900">{{ $case->case_no }}</td>
                    <td class="px-5 py-4 text-gray-700">{{ $case->applicant_name }}</td>
                    <td class="px-5 py-4 text-gray-600">{{ $case->type_of_application }}</td>
                    <td class="px-5 py-4 text-gray-600">{{ $case->denomination }}</td>
                    <td class="px-5 py-4 text-gray-600">{{ formatDate($case->date_filed) }}</td>
                    <td class="px-5 py-4 text-gray-600">{{ $case->expiry_date ? formatDate($case->expiry_date) : '—' }}</td>
                    <td class="px-5 py-4">
                        <span class="px-2 py-1 text-xs rounded-full {{ $badgeClass }}">{{ $badgeText }}</span>
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            <button onclick='openEditModal(@json($case))' class="text-blue-600 hover:text-blue-900">
                                <i data-lucide="edit" class="w-4 h-4"></i>
                            </button>
                            <form method="POST" action="{{ route('decision-management.destroy', $case->id) }}" onsubmit="return confirm('Delete this case?')">
                                @csrf @method('DELETE')
    ...
    <div class="px-5 py-4 border-t flex items-center justify-between text-sm text-gray-600">
        ...
        <div class="flex gap-2">
            @if($pagination['has_prev'])
                <a href="?page={{ $pagination['prev_page'] }}&search={{ $search }}" class="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200">← Prev</a>
            @endif
            @if($pagination['has_next'])
                <a href="?page={{ $pagination['next_page'] }}&search={{ $search }}" class="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200">Next →</a>
            @endif
        </div>
    </div>
    @endif
...
<div id="addCaseModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
        ...
        <form method="POST" action="{{ route('decision-management.store') }}">
            @csrf
            <div class="space-y-3">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_filed" value="{{ date('Y-m-d') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500 focus:outline-none" required>
                    </div>
                </div>
                <div>
...
function openEditModal(c) {
    const base = "{{ url('decision-management') }}";
