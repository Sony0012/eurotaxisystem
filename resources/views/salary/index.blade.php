...
<div id="printableReportContainer" class="hidden bg-white text-black p-8 w-full min-h-screen">
    ...
    <div class="mb-8 flex justify-between text-sm">
        <div>
            <p><strong class="font-bold text-gray-800">Generated By:</strong> {{ auth()->user()->full_name ?? 'Admin' }}</p>
            <p><strong class="font-bold text-gray-800">Date Generated:</strong> {{ date('F d, Y h:i A') }}</p>
        </div>
        <div class="text-right">
            <p><strong class="font-bold text-gray-800">Total Employees:</strong> {{ $summary['total_employees'] ?? 0 }}</p>
            <p><strong class="font-bold text-gray-800">Net Profit (After Payroll):</strong> {{ formatCurrency($summary['net_profit'] ?? 0) }}</p>
        </div>
    </div>
    ...
    <table class="w-full text-left border-collapse mb-8 text-sm">
        <thead>
            <tr class="bg-gray-100 border-y-2 border-gray-800">
                <th class="py-3 px-2 font-bold uppercase text-gray-800">Employee</th>
                <th class="py-3 px-2 font-bold uppercase text-gray-800">Type</th>
                <th class="py-3 px-2 font-bold uppercase text-gray-800 text-right">Basic Salary</th>
                <th class="py-3 px-2 font-bold uppercase text-gray-800 text-right">Overtime</th>
                <th class="py-3 px-2 font-bold uppercase text-gray-800 text-right">Net Total</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-300">
            ...
            <tr>
                <td class="py-2 px-2 text-gray-900">
                    <span class="font-bold">{{ $salary->employee_name }}</span><br>
                    <span class="text-xs text-gray-500">{{ $salary->position ?? '' }}</span>
                </td>
                <td class="py-2 px-2 text-gray-900">{{ ucfirst($salary->salary_type ?? 'Monthly') }}</td>
                <td class="py-2 px-2 text-right text-gray-900">{{ formatCurrency($salary->basic_salary) }}</td>
                <td class="py-2 px-2 text-right text-gray-900">{{ formatCurrency($salary->overtime_pay ?? 0) }}</td>
                <td class="py-2 px-2 text-right font-bold text-gray-900">{{ formatCurrency($salary->total_pay ?? $salary->basic_salary) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
