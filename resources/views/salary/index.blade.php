...
<div id="printableReportContainer" class="hidden bg-white text-black w-full font-sans" style="font-family: Arial, sans-serif; font-size: 12px;">
    ...
    <div style="text-align:center; border-bottom: 2px solid #000; padding-bottom: 12px; margin-bottom: 12px;">
        <img src="{{ asset('uploads/logo.png') }}" alt="Euro Taxi System" style="height:64px; display:block; margin:0 auto 6px auto; object-fit:contain;" onerror="this.style.display='none'">
        <div style="font-size:14px; font-weight:bold; letter-spacing:2px; text-transform:uppercase; color:#000; margin-bottom:2px;">Monthly Salary Report</div>
        <div style="font-size:11px; color:#444;">Period: {{ \Carbon\Carbon::parse($date_from)->format('F d, Y') }} &mdash; {{ \Carbon\Carbon::parse($date_to)->format('F d, Y') }}</div>
    </div>
    ...
    <table style="width:100%; border-collapse:collapse; font-size:11px; margin-bottom:16px;">
        <thead>
            <tr style="background:#f0f0f0; border-top:2px solid #000; border-bottom:2px solid #000;">
                <th style="padding:7px 6px; text-align:left; font-weight:bold; text-transform:uppercase;">Employee</th>
                <th style="padding:7px 6px; text-align:left; font-weight:bold; text-transform:uppercase;">Type</th>
                <th style="padding:7px 6px; text-align:right; font-weight:bold; text-transform:uppercase;">Basic Salary</th>
                <th style="padding:7px 6px; text-align:right; font-weight:bold; text-transform:uppercase;">Overtime</th>
                <th style="padding:7px 6px; text-align:right; font-weight:bold; text-transform:uppercase;">Net Total</th>
            </tr>
        </thead>
        <tbody>
            @foreach($salaries as $i => $salary)
            <tr style="border-bottom: 1px solid #ccc; background: {{ $i % 2 === 0 ? '#fff' : '#fafafa' }};">
                <td style="padding:6px 6px;">
                    <span style="font-weight:bold;">{{ $salary->employee_name }}</span><br>
                    <span style="font-size:10px; color:#555;">{{ $salary->position ?? '' }}</span>
                </td>
                <td style="padding:6px 6px;">{{ ucfirst($salary->salary_type ?? 'Monthly') }}</td>
                <td style="padding:6px 6px; text-align:right;">{{ formatCurrency($salary->basic_salary) }}</td>
                <td style="padding:6px 6px; text-align:right;">{{ formatCurrency($salary->overtime_pay ?? 0) }}</td>
                <td style="padding:6px 6px; text-align:right; font-weight:bold;">{{ formatCurrency($salary->total_pay ?? $salary->basic_salary) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr style="border-top:3px solid #000; background:#eee;">
                <td colspan="2" style="padding:8px 6px; text-align:right; font-weight:900; text-transform:uppercase;">Grand Total</td>
                <td style="padding:8px 6px; text-align:right; font-weight:900;">{{ formatCurrency($salaries->sum('basic_salary')) }}</td>
                <td style="padding:8px 6px; text-align:right; font-weight:900;">{{ formatCurrency($salaries->sum('overtime_pay')) }}</td>
                <td style="padding:8px 6px; text-align:right; font-weight:900; font-size:13px;">{{ formatCurrency($summary['total_salaries'] ?? 0) }}</td>
            </tr>
        </tfoot>
    </table>
