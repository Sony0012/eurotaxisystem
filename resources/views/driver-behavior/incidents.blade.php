...
<div>
    ...
    <div id="incidentsTableContainer">
        @include('driver-behavior.partials._incidents_table')
    </div>
...
// ─── AJAX Search functionality for Incidents Table ───
document.addEventListener('DOMContentLoaded', () => {
    let searchTimer;
    ...
    function performSearch(page = 1) {
        ...
        const finalUrl = filterForm.action + '?' + params;
    ...
    if (searchInput) {
        // Remove native form submission on enter since JS will handle it
    ...
    // Bind other filters to AJAX search
    ['select[name="type"]', 'select[name="severity"]', 'input[name="date_from"]', 'input[name="date_to"]'].forEach(selector => {
        const el = document.querySelector(selector);
        if (el) el.addEventListener('change', () => performSearch(1));
    });
