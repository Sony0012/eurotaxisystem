...
@section('content')
    ...
    <div id="addUnitModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 max-h-screen overflow-y-auto">
            ...
            <form method="POST" action="{{ route('units.store') }}" id="addUnitForm">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="driver_id"
                            ...
                            @foreach($all_drivers as $driver)
                                <option value="{{ $driver->id }}">{{ $driver->full_name }} ({{ $driver->contact_number }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="driver_id" id="editDriver1"
                            ...
                            @foreach($all_drivers as $d)
                                <option value="{{ $d->id }}">{{ $d->full_name }} ({{ $d->contact_number }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
