    ...
    <table class="min-w-full text-sm modern-table-sep">
        ...
        <tbody>
            @forelse($units as $unit)
                ...
                <tr class="{{ $has_maintenance_data ? 'modern-row-has-sub' : 'modern-row' }} cursor-pointer group" onclick="viewUnitDetails({{ $unit->uuid }})">
                    ...
                    <td class="px-2 md:px-6 py-3 md:py-5 whitespace-nowrap text-center relative z-30" onclick="event.stopPropagation()">
                        ...
                        <div id="unit-dropdown-{{ $unit->uuid }}"
                            class="unit-action-dropdown hidden absolute right-2 top-full mt-1 w-44 bg-white rounded-xl shadow-2xl border border-gray-200 z-[999999] overflow-hidden" style="z-index: 999999 !important;">
                            {{-- Edit --}}
                            <button type="button"
                ...
                @if($has_maintenance_data)
                    <tr class="modern-sub-row cursor-pointer relative z-1" style="z-index: 1 !important;" onclick="viewUnitDetails({{ $unit->uuid }})">
