        ...
        <tbody>
            @forelse($units as $unit)
                ...
                <tr class="{{ $has_maintenance_data ? 'modern-row-has-sub' : 'modern-row' }} cursor-pointer group" onclick="viewUnitDetails({{ $unit->uuid }})">
                    ...
                    {{-- Actions --}}
                    <td class="px-2 md:px-6 py-3 md:py-5 whitespace-nowrap text-center relative z-20">
                        <button type="button"
                        ...
                        <div id="unit-dropdown-{{ $unit->uuid }}"
                            class="unit-action-dropdown hidden absolute right-2 mt-1 w-44 bg-white rounded-xl shadow-2xl border border-gray-200 z-[99999] overflow-hidden">
                            {{-- Edit --}}
                            <button type="button"
