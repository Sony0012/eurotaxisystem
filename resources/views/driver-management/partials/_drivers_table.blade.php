<div class="overflow-x-auto pb-4">
    <table class="min-w-full text-sm modern-table-sep">
        <thead class="bg-gray-50/80 border-b border-gray-100">
            <tr>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest w-1/4">
                    Driver Profile</th>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Assigned
                    Unit</th>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">License
                    Detail</th>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Status
                </th>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Financial
                    Target</th>
                <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Rating
                </th>
                <th
                    class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">
                    Actions</th>
            </tr>
        </thead>
        <tbody>
                ...
                <tr class="modern-row cursor-pointer group {{ $has_shortage ? 'shortage-row' : '' }}"
                    ...
                    <td class="px-6 py-5">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 rounded-full {{ $has_shortage ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600' }} flex items-center justify-center flex-shrink-0 shadow-inner">
                                <span
                                    class="text-lg font-black">{{ substr($driver->first_name ?? $driver->full_name, 0, 1) }}{{ substr($driver->last_name ?? '', 0, 1) }}</span>
                            </div>
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <h4
                                        class="text-sm font-black {{ $has_shortage ? 'text-red-700 shortage-text-blink' : 'text-gray-900' }}">
                                        {{ $driver->full_name }}
                                    </h4>
                                    @if($has_shortage)
                    ...
                    <td class="px-6 py-5 whitespace-nowrap">
                        @if(!empty($driver->assigned_unit))
                            <div
                                class="inline-flex items-center gap-2 bg-slate-800 text-white px-3 py-1.5 rounded-lg shadow-sm">
                                <i data-lucide="car" class="w-4 h-4 text-blue-400"></i>
                                <span class="text-sm font-black tracking-widest">{{ $driver->assigned_unit }}</span>
                            </div>
                        @else
                            <span
                                class="inline-flex items-center gap-1.5 text-emerald-700 font-black text-[11px] bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200 uppercase tracking-widest">
                    ...
                    <td class="px-6 py-5 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            @if($driver->driver_status === 'banned')
                                ...
                                <span
                                    class="text-[11px] font-black uppercase tracking-widest text-red-600 flex items-center gap-1">
                                    <i data-lucide="ban" class="w-3 h-3"></i> Banned
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full {{ $driver->is_active ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
                                    <div
                    ...
                    <td class="px-6 py-5 whitespace-nowrap text-right relative">
                        <button type="button"
                            class="p-2 text-gray-400 hover:text-gray-800 hover:bg-gray-200 rounded-full transition-colors focus:outline-none"
