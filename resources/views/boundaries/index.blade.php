...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form method="GET" action="{{ route('boundaries.index') }}" class="flex flex-col lg:flex-row gap-4">
            <div class="flex-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-5">
            ...
            <p class="text-2xl font-bold text-green-600">{{ formatCurrency($totals->today_total ?? 0) }}</p>
            <p class="text-xs text-gray-400">{{ date('M d, Y') }}</p>
        </div>
        <div class="bg-white rounded-lg shadow p-5">
            <p class="text-sm text-gray-500">Total Collected</p>
            <p class="text-2xl font-bold text-blue-600">{{ formatCurrency($totals->total_amount ?? 0) }}</p>
            <p class="text-xs text-gray-400">{{ $totals->total_records ?? 0 }} records</p>
        </div>
        <div class="bg-white rounded-lg shadow p-5">
            <p class="text-sm text-gray-500">Shortage</p>
            <p class="text-2xl font-bold text-red-600">{{ formatCurrency($totals->total_shortage ?? 0) }}</p>
            <p class="text-xs text-gray-400">Deficit amount</p>
        </div>
        <div class="bg-white rounded-lg shadow p-5">
            <p class="text-sm text-gray-500">Period</p>
            <p class="text-base font-bold text-gray-900">{{ formatDate($date_from) }} – {{ formatDate($date_to) }}</p>
        </div>
    </div>
                    ...
                    @forelse($boundaries as $b)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                {{ formatCurrency($b->boundary_amount ?? 0) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ formatCurrency($actual) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold {{ $diff >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full
                                        ...
                                        @elseif($b->status === 'excess') bg-blue-100 text-blue-800
